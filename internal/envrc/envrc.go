...
type Config struct {
	...
	// When non-empty, "bwenv export" will only fetch custom fields from these
	// items instead of all items in the folder. Empty means load all items.
	ItemIDs []string `json:"item_ids,omitempty"`

	// ItemNames are the human-readable item names (for display in .envrc header only).
...
func Generate(cfg Config) error {
	...
	if len(cfg.ItemNames) > 0 {
		b.WriteString(fmt.Sprintf("# Provider: %s | Folder: %s | Items: %s\n",
			cfg.ProviderSlug, cfg.FolderName, strings.Join(cfg.ItemNames, ", ")))
	} else {
...
func AllowAndExport() (providerSlug string, folderName string, err error) {
	...
	providerSlug, folderName, _, err = ParseEnvrcConfig()
	if err != nil {
...
// the provider slug, folder name, and optional item IDs. Returns an error
// if the file doesn't exist or doesn't appear to be generated by bwenv.
func ParseEnvrcConfig() (providerSlug string, folderName string, itemIDs []string, err error) {
	providerSlug, folderName, _, itemIDs, err = ParseEnvrcConfigWithFolderID()
	return providerSlug, folderName, itemIDs, err
}
...
func ParseEnvrcConfigWithFolderID() (providerSlug string, folderName string, folderID string, itemIDs []string, err error) {
	...
	if !strings.Contains(contentStr, "bwenv") {
		return "", "", "", nil, fmt.Errorf(".envrc was not generated by bwenv")
	}

	// Extract the machine identifier independently of the header so generated
	// files can keep the human-readable folder metadata unchanged.
	for _, line := range strings.Split(contentStr, "\n") {
		line = strings.TrimSpace(line)
		if !strings.Contains(line, "bwenv export") {
			continue
		}
		if idx := strings.Index(line, "--folder-id"); idx >= 0 {
			rest := strings.TrimSpace(line[idx+len("--folder-id"):])
			...
			if len(rest) > 0 && (rest[0] == '\'' || rest[0] == '"') {
				...
				if end := strings.IndexByte(rest[1:], quote); end >= 0 {
					folderID = rest[1 : end+1]
				}
			} else if fields := strings.Fields(rest); len(fields) > 0 {
				folderID = strings.Trim(fields[0], "'\"")
			}
		}
		break
	}
	...
	if providerSlug == "" || folderName == "" {
		for _, line := range strings.Split(contentStr, "\n") {
			...
			if strings.Contains(line, "bwenv export") {
				...
				if idx := strings.Index(line, "--items"); idx >= 0 {
					rest := line[idx+len("--items"):]
					rest = strings.TrimSpace(rest)
					if len(rest) > 0 && (rest[0] == '\'' || rest[0] == '"') {
						quote := rest[0]
						end := strings.IndexByte(rest[1:], quote)
						if end >= 0 {
							itemsStr := rest[1 : end+1]
						...
						fields := strings.Fields(rest)
						if len(fields) > 0 {
							itemsStr := strings.TrimSuffix(fields[0], ")")
	...
	// Item names in the header are display-only. Prefer the actual IDs from
	// the generated command, including when provider and folder came from the
	// header and the legacy fallback above was skipped.
	for _, line := range strings.Split(contentStr, "\n") {
		line = strings.TrimSpace(line)
		if !strings.Contains(line, "bwenv export") {
			continue
		}
		if itemsValue := extractShellFlag(line, "--items"); itemsValue != "" {
			itemIDs = nil
			for _, id := range strings.Split(itemsValue, ",") {
				id = strings.TrimSpace(id)
				if id != "" {
					itemIDs = append(itemIDs, id)
				}
	...
	if providerSlug == "" || folderName == "" {
		return providerSlug, folderName, folderID, nil, fmt.Errorf("could not extract provider and folder from .envrc")
	}

	return providerSlug, folderName, folderID, itemIDs, nil
}

func extractShellFlag(line string, flag string) string {
