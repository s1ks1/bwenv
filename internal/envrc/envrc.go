...
func Generate(cfg Config) error {
	...
	if len(cfg.ItemIDs) > 0 {
		...
		b.WriteString(fmt.Sprintf("eval \"$(bwenv export --provider %s --folder %s --items %s)\"\n",
...
func ParseEnvrcConfig() (providerSlug string, folderName string, itemIDs []string, err error) {
	...
	for _, line := range strings.Split(contentStr, "\n") {
		...
		if strings.HasPrefix(line, "# Provider:") {
			...
			if len(parts) >= 3 {
				itemsPart := strings.TrimPrefix(parts[2], " Items:")
				itemsPart = strings.TrimSpace(itemsPart)
				if itemsPart != "" {
					for _, name := range strings.Split(itemsPart, ",") {
						name = strings.TrimSpace(name)
						if name != "" {
							itemIDs = append(itemIDs, name)
						}
					}
				}
			}
	...
	if providerSlug == "" || folderName == "" {
		return providerSlug, folderName, nil, fmt.Errorf("could not extract provider and folder from .envrc")
	}

	return providerSlug, folderName, itemIDs, nil
}
...
func Export(providerSlug string, folderName string, itemIDs []string) error {
	_, err := exportSecrets(providerSlug, folderName, itemIDs, false)
	return err
}
...
func exportSecrets(providerSlug string, folderName string, itemIDs []string, interactive bool) (string, error) {
...
func PreviewSecretsByIDs(p provider.Provider, session string, itemIDs []string) ([]string, error) {
	secrets, err := p.GetSecretsByItemIDs(session, itemIDs)
	if err != nil {
		return nil, err
	}
